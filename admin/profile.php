...
// 🛠️ اصلاح شد: استفاده از متد واقعی و موجود در پروژه شما
...
<head>
    ...
    <title>پروفایل مدیریت | سامانه رزرو استادیوم</title>
...
<div class="panel-main" id="panelMain">
    ...
    <div class="panel-content">
        ...
        <div class="page-header">
            <h1>تنظیمات پروفایل مدیریت</h1>
            <div class="breadcrumb-nav">
                <a href="dashboard.php">داشبورد مدیریت</a>
            ...
            <div class="col-lg-7">
                <div class="panel-card">
                    <div class="panel-card-header">
                        <h6><i class="fas fa-user-shield ms-2"></i> ویرایش مشخصات مدیر</h6>
                    </div>
                    <div class="panel-card-body">

                        <div class="profile-avatar-wrap text-center mb-4">
                            <div class="profile-avatar mx-auto" style="width: 110px; height: 110px; border-radius: 50%; position: relative; background: #1a1f3c; display: flex; align-items: center; justify-content: center; border: 2px solid #39FF14;">
                                ...
                                <label for="avatarInput" class="avatar-upload-btn" title="تغییر تصویر" style="cursor: pointer; position: absolute; bottom: 4px; right: 4px; z-index: 5; background: #00F0FF; color: #0a0e27; width: 30px; height: 30px; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: 2px solid #1a1f3c; margin: 0; box-shadow: 0 0 10px rgba(0, 240, 255, 0.5);">
...
<script>
        ...
        avatarInput.addEventListener('change', async e => {
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    window.Popup?.error('خطای سرور', `کد خطا: ${res.status}`);
                    return;
                }
                ...
                try { data = JSON.parse(rawText); } catch(jsonErr) {
                    window.Popup?.error('خطای ساختار بک‌اند', 'پاسخ سرور نامعتبر است.');
                    return;
                }

                if (data.success) {
                    window.Popup?.success('موفقیت', data.message);
                    if (data.avatar_url) {
                        ...
                        if (document.querySelector('.profile-avatar-placeholder')) {
                            location.reload();
                        }
                    } else {
                ...
                window.Popup?.error('خطای ارتباطی', 'مشکلی در ارسال داده‌ها پیش آمد.');
            } finally {
                e.target.value = '';
            }
        });
    ...
    document.getElementById('passwordForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (data.success) {
                window.Popup?.success('بروزرسانی', data.message);
                e.target.reset();
            } else {
                window.Popup?.error('خطا', data.message);
            }
